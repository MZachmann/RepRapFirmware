...
/* Screw Mapping is a well-known software repair mechanism for screw nonlinearities in axes
	...
	This code, instead, permits a piecewise-linear multi-axis transformation of the entire machining space,
	as if you were holding a flexible cube and pulling/pushing/twisting/poking it

	Unfortunately, a full multivariate grid transform would take lots of RAM so instead
	this code does multiple **one axis to many** transforms
	This method allows correction for many physical defects - in my case not-totally-straight rails

	--Storage

	A Screwmap object is a bunch of ScrewMapInfo (smi) objects, one per source axis defining the transforms

	smi.usedAxes is the list of axes that get adjusted for each specific input axis
	smi.mapTable is a float array with one row per adjusted axis, with axis delta values in each entry

	When the map table is created you define Start, Increment, and Count for the independent variable
	As well as a string list of which axes are transformed

	So, if the Y axis has Start=10, Increment=1, Count=4 and it adjusts 'XZ' the map table has two rows 
	of 4 deltas applied at 10,11,12,13
...
// find the row corresponding to an axis by index (letter)
...
bool ScrewMap::SetScrewAxis(unsigned int srcAxis, unsigned int destAxis, float* deltas, size_t count, size_t dataOffset) noexcept
{
	...
	{
		reprap.GetPlatform().MessageF(ErrorMessage, "Attempt to set unused screw map row %d:%d with map %ld\n", srcAxis, destAxis, smi.usedAxes.GetRaw() );
	}
...
void ScrewMap::UpdatePositions() noexcept
{
	...
	InvalidateCache();
	// reprap.GetGCodes().GetMovementState(0).AdjustMotorPositions(vs, MaxAxes);	// steps -> machine pos
